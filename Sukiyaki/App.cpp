...
bool App::Loop()
{
	...
	//画面のクリア
	if (FAILED(d3d->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(128, 128, 128), 1.0f, 0)))
		return false;
...
//ウィンドウプロシージャ
LRESULT CALLBACK App::winProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_DESTROY:
...
//ウィンドウの初期化
bool App::InitWindow(HINSTANCE hInst, int cmdShow)
{
	//ウィンドウクラスの初期化
	WNDCLASSEX winClass;
...
//Direct3Dデバイスの初期化
bool App::InitD3dDevice()
{
	//ディスプレイアダプタを表すためのデバイスを作成
	//描画と頂点処理をハードウェアで行なう
	if (FAILED(direct3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &presentParam, &d3d)))
		//上記の設定が失敗したら
		//描画をハードウェアで行い、頂点処理はCPUで行なう
		if (FAILED(direct3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &presentParam, &d3d)))
			//上記の設定が失敗したら
			//描画と頂点処理をCPUで行なう
			if (FAILED(direct3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF, hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &presentParam, &d3d)))
				return false;

	//レンダリングステータスの設定
	d3d->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);	//両面描画モードの設定
	d3d->SetRenderState(D3DRS_ZENABLE, true);				//Z比較を行う
	d3d->SetRenderState(D3DRS_DITHERENABLE, true);		//ディザリングを行う
	...
	d3d->SetRenderState(D3DRS_POINTSIZE, FtoDW(1.0f));
	d3d->SetRenderState(D3DRS_POINTSIZE_MIN, FtoDW(0.0f));
	d3d->SetRenderState(D3DRS_POINTSCALE_A, FtoDW(0.0f));
	d3d->SetRenderState(D3DRS_POINTSCALE_B, FtoDW(0.0f));
	d3d->SetRenderState(D3DRS_POINTSCALE_C, FtoDW(1.0f));

	return true;
}

//DirectInputの初期化
bool App::InitDirectInput(HINSTANCE hInstance)
{
	//DirectInputオブジェクトの作成
	if (FAILED(DirectInput8Create(hInstance, DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&dInput, nullptr)))
		return false;
...
//DirectInputデバイスの初期化
bool App::InitDinputDevice()
{
	//DirectInputデバイスを作成
	if (FAILED(dInput->CreateDevice(GUID_SysMouse, &dInputDevice, nullptr)))
		return false;
